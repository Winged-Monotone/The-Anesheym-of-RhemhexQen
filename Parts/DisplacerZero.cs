...
namespace XRL.World.Parts 
{
    ...
    public class DisplacerZero : IPart
    {
        public static void DisplaceBetweenFloors0()
        {
                ...
                if (!NeutronFluxDetonation.NFDGoesBoom)
                {
                    if (The.Game.HasFinishedQuestStep("ShahStartLongSigh", "LSSpeakwithSaile"))
                    {
                    ...
                    {
                        ThePlayer.ShowFailure("The panel refuses to respond.");
                    }
                }
                else
                {
                    ThePlayer.ShowFailure("Destination can't be found.");
                }
        }
    }
}
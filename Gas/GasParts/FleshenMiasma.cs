...
{
    [Serializable]
    public class FleshenMiasma : IPart
    {
        ...
        {
            FleshenMiasma FleshenMiasma = p as FleshenMiasma;
            if (FleshenMiasma.GasType != GasType)
            {
                return false;
            }

            if (FleshenMiasma.GasLevel != GasLevel)
            {
                return false;
            }
            ...
            if (FleshenMiasma.Noun != Noun)
            {
                return false;
            }

            if (FleshenMiasma.MessageColor != MessageColor)
            {
                return false;
            }

            if (FleshenMiasma.DamageAttributes != DamageAttributes)
            {
                return false;
            }

            if (FleshenMiasma.TargetPart != TargetPart)
            {
                return false;
            }

            if (FleshenMiasma.TargetTag != TargetTag)
            {
                return false;
            }

            if (FleshenMiasma.ExcludeTag != ExcludeTag)
            {
                return false;
            }

            if (FleshenMiasma.TargetTagValue != TargetTagValue)
            {
                return false;
            }

            if (FleshenMiasma.TargetEquippedPart != TargetEquippedPart)
            {
                return false;
            }

            if (FleshenMiasma.TargetEquippedTag != TargetEquippedTag)
            {
                return false;
            }

            if (FleshenMiasma.TargetEquippedTagValue != TargetEquippedTagValue)
            {
                return false;
            }

            if (FleshenMiasma.TargetBodyPartCategoryCode != TargetBodyPartCategoryCode)
            {
                return false;
            }

            if (FleshenMiasma.AffectEquipment != AffectEquipment)
            {
                return false;
            }

            if (FleshenMiasma.AffectCybernetics != AffectCybernetics)
            {
                return false;
            }

            if (FleshenMiasma.Respiratory != Respiratory)
            {
                return true;
            }
        ...
        public override bool HandleEvent(GetNavigationWeightEvent E)
        {
            if (CheckGasCanAffectEvent.Check(E.Actor, ParentObject))
            {
                if (E.Smart)
                {
                    if (E.Actor == null || (E.Actor.FireEvent("CanApplyFleshenMiasmaEffect") &&
        ...
        public void ApplyFleshenMiasma()
        {
            ApplyFleshenMiasma(ParentObject.CurrentCell);
        }

        public void ApplyFleshenMiasma(Cell C)
        {
            if (C == null)
            {
                return;
            ...
            for (int count = list.Count; i < count; i++)
            {
                if (ApplyFleshenMiasma(list[i]) && list == C.GetObjectsInCell())
                {
        ...
        public bool ApplyFleshenMiasma(GameObject GO)
        {
            if (GO == ParentObject)
            {
                return false;
            ...
            // if (!GO.FireEvent("CanApplyFleshenMiasmaEffect"))
            // {
            //     return false;
            // }
            //
            // if (!CanApplyEffectEvent.Check(GO, "FleshenMiasmaEffect"))
            // {
            //     return false;
            // }
            ...
            // if (!GO.HasEffect("FleshenMiasmaEffect"))
            // {
            //     // FleshenMiasmaEffect FleshenMiasmaEffect = new FleshenMiasmaEffect(Stat.Random(15, 25),
            //         (
            //             +Stat.Random(1, 10) + GasLevel + (GasLevel * 4)
            //         ),
            //         gas.Creator);
            //     // GO.ApplyEffect(FleshenMiasmaEffect);
            //     return false;
            // }
            // else
